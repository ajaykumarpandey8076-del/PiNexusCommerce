// --- Render Compact AI Commerce Assistant Widget (Foolproof Hidden State) ---
function renderCommerceAssistant() {
  ...
  if (!assistantContainer) {
    ...
    assistantContainer.className = 'bg-white border border-purple-100 rounded-2xl p-4 shadow-sm my-4 max-w-md mx-auto';
    
    const searchSection = document.getElementById('searchInput')?.closest('div') || document.getElementById('opportunities-container');
    if (searchSection && searchSection.parentNode) {
      searchSection.parentNode.insertBefore(assistantContainer, searchSection);
    } else {
  ...
  assistantContainer.innerHTML = `
    ...
    <!-- Foolproof default hidden state using inline style display: none -->
...
// --- Voice Recognition Handler (Explicitly shows/hides status on click) ---
...
function toggleVoiceRecording() {
  ...
  // Show status only when recording starts via button click
  if (statusEl) {
